...
### 요구사항
- 로그인 한 사용자는 쿠폰을 사용하여 주문 시 할인 혜택을 받을 수 있다.
- 쿠폰 종류는 정액 / 정률 할인 쿠폰으로 구분된다.
- 정액 할인 쿠폰은 주문 금액에서 고정 금액을 차감한다.
- 정률 할인 쿠폰은 주문 금액에서 일정 비율을 차감한다.
- 쿠폰 사용 시 쿠폰의 유효성(사용 가능 여부, 유효 기간 등)을 검증한다.
- 쿠폰 사용 후 주문 금액에서 쿠폰 할인 금액을 차감한 최종 결제 금액을 계산한다.
- 쿠폰 사용 내역은 주문 상세 정보에 포함되어야 한다.
- 발급 된 쿠폰은 한 번만 사용 가능하며, 사용 후에는 더 이상 사용할 수 없도록 상태를 변경한다.

**실패 케이스**
- 로그인하지 않은 사용자가 쿠폰을 사용하려고 하면 오류를 반환한다.
- 요청된 쿠폰 ID에 해당하는 쿠폰이 존재하지 않으면 오류를 반환한다.
- 쿠폰이 이미 사용된 상태이거나 유효하지 않은 경우 오류를 반환한다.
- 쿠폰 사용 시 주문 금액이 쿠폰 할인 금액보다 적은 경우 오류를 반환한다.

**HTTP 상태코드 / 에러메시지 정의**
| 케이스 | 설명 | HTTP 상태코드 | 에러메시지 예시 |
| --- | --- | --- | --- |
| 사용자 인증 정보 불충분/오류 | 로그인하지 않은 사용자가 쿠폰 사용 시도 | 401 UNAUTHORIZATION | "로그인이 필요합니다." |
| 쿠폰 미존재 | 존재하지 않는 쿠폰 ID 요청 | 404 NOT FOUND | "해당 쿠폰을 찾을 수 없습니다." |
| 쿠폰 사용 불가 | 쿠폰이 이미 사용되었거나 유효하지 않은 경우 | 400 BAD REQUEST | "해당 쿠폰은 사용할 수 없습니다." |
|| 쿠폰 할인 금액 초과 | 주문 금액이 쿠폰 할인 금액보다 적은 경우 | 400 BAD REQUEST | "쿠폰 할인 금액이 주문 금액을 초과할 수 없습니다." |
|| 내부 서버 오류 | 쿠폰 사용 처리 중 시스템 오류 | 500 INTERNAL SERVER ERROR | "쿠폰 사용 처리 중 오류가 발생했습니다." |
